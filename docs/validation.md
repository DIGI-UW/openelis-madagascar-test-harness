...
1. **OE → bridge registration** — In webapp logs, expect `Bridge registration complete: N bindings pushed` with **N > 0** when analyzers exist and `ANALYZER_BRIDGE_URL` points at the bridge HTTPS URL (see the locked distro compose file).
...
The locked distro compose file selects images via `${OE_IMAGE_TAG:-develop}` (and `${BRIDGE_IMAGE_TAG:-develop}` for the bridge), so swapping to a local build is one env var:

```bash
./scripts/restart-stack.sh --rebuild
```

That builds webapp, frontend, and demo-tests as `:local`, exports `OE_IMAGE_TAG=local`, and restarts the stack via the locked distro compose plus harness overlays.
...
```bash
docker compose -f "$DISTRO_COMPOSE" -f compose.validate.yaml up -d
```
...
```bash
docker compose -f "$DISTRO_COMPOSE" -f compose.validate.yaml ps
...
4. `docker compose -f "$DISTRO_COMPOSE" -f "$DISTRO_REPO/compose.letsencrypt.yaml" up -d --force-recreate proxy`
...
```bash
COMPOSE_PROFILES=demo docker compose -f "$DISTRO_COMPOSE" -f compose.validate.yaml run --rm demo-tests
```
